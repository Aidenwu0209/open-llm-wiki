#!/usr/bin/env python3
"""Batch-convert a directory of PDFs to Markdown through a local parser or API."""
...
from pdf_to_markdown import DEFAULT_API_URL, DEFAULT_TOKEN_ENV, convert, convert_local_text
...
def convert_one(args: argparse.Namespace, input_path: Path, output_dir: Path) -> int:
    ...
    if parser_choice == "local-text":
        return convert_local_text(convert_args)
...
def main() -> int:
    ...
    for index, pdf in enumerate(pdfs, 1):
        ...
        if args.dry_run:
            parser_choice = "local-text" if args.parser in {"auto", "local-text"} else "layout-api"
            print(f"PLAN {pdf} -> {output_dir} [parser={parser_choice}]")
            continue
        try:
