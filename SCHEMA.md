...
`_state/ingest-plan.jsonl` and `_state/ingest-plan.json` are generated by
`wiki_ingest_plan.py` from `raw/inbox/`, raw source files with parse artifacts,
and existing sources. Each row represents a candidate with an explicit plan
state.
...
| `candidate_source` | string | origin: `inbox`, `raw_source`, `raw_artifact`, or `published` |
| `candidate_sha256` | string | SHA-256 of the candidate source file |
| `artifact_path` | string | vault-relative path to parsed artifact, when present |
| `artifact_sha256` | string | SHA-256 of the parsed artifact, when present |
